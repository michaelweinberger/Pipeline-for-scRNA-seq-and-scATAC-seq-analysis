...
qc_signac <- function(signac_obj, species=c("human","mouse","zebrafish"), 
                      min_count=3000, max_count=30000, min_perc_peaks=15,
                      nucleosome_signal=4, tss_enrichment=3,
                      out_dir, out_name) {
  ...
  # plot TSS enrichment
  signac_obj$high.tss <- ifelse(signac_obj$TSS.enrichment > tss_enrichment, 'High', 'Low')
  ...
  signac_obj$nucleosome_group <- ifelse(signac_obj$nucleosome_signal > nucleosome_signal, 
                                        paste("NS >", nucleosome_signal), 
                                        paste("NS <", nucleosome_signal))
  ...
  if (species %in% c("human","mouse")) {
    signac_obj <- subset(x = signac_obj, subset = nCount_ATAC > min_count &
                                                  ...
                                                  pct_reads_in_peaks > min_perc_peaks &
...
cluster_signac <- function(signac_obj, seurat_obj=NULL, integration_key=NULL, resolution=0.4,
                           n_dims=30, umap_colors_rna=NULL, umap_colors_atac=NULL,
                           out_dir, out_name) {
  ...
  if (!is.null(integration_key)) {
    ...
    signac_obj <- FindNeighbors(signac_obj, reduction = "harmony", dims = 2:n_dims)
    ...
    signac_obj <- RunUMAP(signac_obj, reduction = 'lsi', dims = 2:n_dims)
    ...
    signac_obj <- RunUMAP(signac_obj, reduction = "harmony", dims = 2:n_dims)
    ...
    signac_obj <- FindNeighbors(signac_obj, reduction = 'lsi', dims = 2:n_dims)
    signac_obj <- FindClusters(signac_obj, resolution = resolution, verbose = FALSE, algorithm = 3)
...
min_frag <- as.numeric(min_frag)
min_count <- as.numeric(min_count)
max_count <- as.numeric(max_count)
min_perc_peaks <- as.numeric(min_perc_peaks)
nucleosome_signal <- as.numeric(nucleosome_signal)
tss_enrichment <- as.numeric(tss_enrichment)
n_dims <- as.numeric(n_dims)
...
if (!file.exists(paste(out_dir, "/", project_name, "_signac.rds", sep=""))) {
  if (!file.exists(paste(out_dir, "/", project_name, "_signac_feature_matrix.rds", sep=""))) {
    counts <- create_peak_count_matrix(fragment_file_path=in_file, frag_cutoff=min_frag,
...
if (!file.exists(paste(out_dir, "/", project_name, "_signac_qc.rds", sep=""))) {
  signac_obj <- qc_signac(signac_obj=signac_obj, 
                          species=species,
                          min_count=min_count, 
                          max_count=max_count, 
                          min_perc_peaks=min_perc_peaks,
                          nucleosome_signal=nucleosome_signal, 
...
if (!file.exists(paste(out_dir, "/", project_name, "_signac_qc_analysed.rds", sep=""))) {
  signac_obj <- cluster_signac(signac_obj=signac_obj, 
                               seurat_obj=seurat_obj, 
                               integration_key=harmony_var, 
                               n_dims=n_dims,
...
signac_obj <- umap_seurat(seurat_obj=signac_obj, 
                          out_dir=out_dir, 
